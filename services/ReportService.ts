...
export const getReportsByUser = async (from: string, to?: string) => {
    ...
    try {
        ...
        FROM
        (
        SELECT COUNT(transfer_id) AS 'Internal', YEAR(created) 'Year', WEEK(created) 'Week', UserOriginAccount.username
        FROM transfer
        INNER JOIN account AS OriginAccount ON OriginAccount.account_id = transfer.OriginAccountId
        INNER JOIN account AS DestinyAccount ON DestinyAccount.account_id = transfer.DestinyAccountId
        ...
        AND transfer.created BETWEEN ${from} AND ${to}
        ...
        INNER JOIN
        (
        SELECT COUNT(transfer_id) AS 'External', YEAR(created) 'Year', WEEK(created) 'Week', UserOriginAccount.username
        FROM transfer
        INNER JOIN account AS OriginAccount ON OriginAccount.account_id = transfer.OriginAccountId
        INNER JOIN account AS DestinyAccount ON DestinyAccount.account_id = transfer.DestinyAccountId
        INNER JOIN user AS UserOriginAccount ON UserOriginAccount.user_id = OriginAccount.UserId
